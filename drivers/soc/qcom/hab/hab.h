 ...
 * vmid 0 for host, 1 for pvm, 2 and on for guest
 */
#define DEFAULT_VM_FE_NAME "vm2-"
...
#define MAX_VMID_NAME_SIZE 30 /* make sure concascaded name is less than this value */

#ifndef HABCFG_FILE_PATH
#ifdef __QNXNTO__
#define HABCFG_FILE_PATH "/lib64/"
#else
#define HABCFG_FILE_PATH "/etc/"
#endif
#endif

#define HABCFG_FILE_NAME "hab_config.txt"
...
#define HABCFG_GET_VMID(_local_cfg_, _vmid_) (settings)->vmid_mmid_list[_vmid_].vmid
#define HABCFG_GET_MMID(_local_cfg_, _vmid_, _mmid_) (settings)->vmid_mmid_list[_vmid_].mmid[_mmid_]
#define HABCFG_GET_BE(_local_cfg_, _vmid_, _mmid_) (settings)->vmid_mmid_list[_vmid_].is_listener[_mmid_]

typedef struct hab_header {
	uint32_t id_type_size;
	uint32_t session_id;
	uint32_t signature;
	uint32_t sequence;
} __attribute__((packed)) hab_header;
...
#define HAB_HEADER_SET_SIZE(header, size) \
	((header).id_type_size = ((header).id_type_size & (~HAB_HEADER_SIZE_MASK)) |				\
		(((size) << HAB_HEADER_SIZE_SHIFT) & HAB_HEADER_SIZE_MASK))

#define HAB_HEADER_SET_TYPE(header, type) \
	((header).id_type_size = ((header).id_type_size & (~HAB_HEADER_TYPE_MASK)) |				\
		(((type) << HAB_HEADER_TYPE_SHIFT) & HAB_HEADER_TYPE_MASK))

#define HAB_HEADER_SET_ID(header, id) \
	((header).id_type_size = ((header).id_type_size & (~HAB_HEADER_ID_MASK)) |  \
		((HAB_VCID_GET_ID(id) << HAB_HEADER_ID_SHIFT) \
		& HAB_HEADER_ID_MASK))

#define HAB_HEADER_GET_SIZE(header) \
	(((header).id_type_size & HAB_HEADER_SIZE_MASK) >> HAB_HEADER_SIZE_SHIFT)

#define HAB_HEADER_GET_TYPE(header) \
	(((header).id_type_size & HAB_HEADER_TYPE_MASK) >> HAB_HEADER_TYPE_SHIFT)
...
struct hab_open_node {
	struct hab_open_request request; /* Must be first in this struct! */
	struct list_head node;
	int age;
};
 ...
 */
struct export_desc {
	uint32_t  export_id; /* this ID is legit within one ctx and vc */
	int       readonly;
...
int fill_default_gvm_settings(struct local_vmid *settings, int vmid_local, int mmid_start, int mmid_end);
