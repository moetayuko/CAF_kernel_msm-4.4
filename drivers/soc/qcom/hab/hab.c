		...
		unsigned int mm_id)
{
	...
	while (1) {
		...
		if (ret || !recv_request) {
			/* device is closed */
			HAB_LOG_ERR("open request wait failed ctx closing %d, listen pchan %p\n",
		...
		ret = hab_open_listen(ctx, dev, &request, &recv_request, 10*1000*1000);
				...
				unsigned int flags)
{
	...
	/* Non-blocking mode is supported only in kernel context */
		...
		uint32_t flags)
{
	...
	if (hab_is_loopback()) {
		...
		//ToDo: use device_table
		int i;
		struct hab_device *dev = NULL;

		//ToDo: how to handle two guestVMs with same mmid? Here assume mmID are different
		// if there are actual two guestVMs, the first vm will be served, then the second one
		for (i = 0; i < hab_driver.ndevices; i++) {
			if (hab_driver.devp[i].id == HAB_MMID_GET_MAJOR(mmid)) {
				dev = &hab_driver.devp[i];
				break;
			}
		}

		if (dev) {
			/* ToDo: multi-gvm find the first pchan available? This works
			   if FE only talks to BE, and BE's pchans are all BE/listener */
			struct physical_channel *pchan = hab_pchan_find_domid(dev, HABCFG_VMID_DONT_CARE);

			if (pchan->is_be)
				vchan = backend_listen(ctx, mmid);
			else
				vchan = frontend_open(ctx, mmid, HABCFG_VMID_DONT_CARE);
		} else {
			HAB_LOG_ERR("failed to find hab-device based on mmid %d\n", mmid);
		}
	}
 ...
 *   vmid (self)
 */
static int hab_initialize_pchan_entry(struct hab_device *mmid_device, int vmid_local, int vmid_remote, int is_be)
{
	...
	if (!mmid_device) {
		HAB_LOG_ERR("wrong input parameters habdev %p, vmid local %d, remote %d, is be %d\n",
					mmid_device, vmid_local, vmid_remote, is_be);
		return -EINVAL;
	}

	sprintf(pchan_name, "vm%d-", vmid);
	strcat(pchan_name, mmid_device->name);

	ret = habhyp_commdev_alloc((void**)&pchan, is_be, pchan_name, vmid_remote, mmid_device);
	if (0 == ret) {
		HAB_LOG_INFO("pchan %s, vmid local(%d) remote(%d), is_be %d added. total %d\n",
					 pchan_name, vmid_local, vmid_remote, is_be, mmid_device->pchan_cnt);
	} else {
		HAB_LOG_ERR("failed %d to allocate pchan %s, vmid local(%d) remote(%d), is_be %d, total pchans %d\n",
				  ret, pchan_name, vmid_local, vmid_remote, is_be, mmid_device->pchan_cnt);
	}
	//mmid_device->id = mmid;
	//pchan_entry->is_be = is_be;
	//	INIT_LIST_HEAD(&pchan_entry->pchannels);
	//spin_lock_init(&pchan_entry->pchan_lock);
	//INIT_LIST_HEAD(&pchan_entry->openq_list);
	//spin_lock_init(&pchan_entry->openlock);
	return ret;
}
	...
	for (i = 0; i<HABCFG_VMID_MAX; i++) {
		if (HABCFG_GET_VMID(settings, i) != HABCFG_VMID_INVALID &&
			...
			for (j=1; j<=HABCFG_MMID_AREA_MAX; j++) {
				if (HABCFG_GET_MMID(settings, i, j) != HABCFG_VMID_INVALID) {
					HAB_LOG_INFO("find valid %d as mmid %d in vmid %d \n", HABCFG_GET_MMID(settings, i, j), j, i);
					switch (HABCFG_GET_MMID(settings, i, j)) {
					case MM_AUD_START/100:
						for (k=MM_AUD_START+1; k<MM_AUD_END; k++) {
							/* if this local pchan end is BE, then use remote FE's vmid. If local end is FE, then use self vmid */
							ret += hab_initialize_pchan_entry(get_hab_device_by_mmid(k),
													   settings->self,
					...
					default:
						HAB_LOG_ERR("failed to find mmid as %d based on vmid %d, mmid %d\n",
...
int do_hab_parse(void)
{
	...
	if (result) {
		HAB_LOG_ERR("failed to register hypervisor plug-in ret %d!\n", result);
		return result;
	}
	...
	memset(&hab_driver.settings, HABCFG_VMID_INVALID, sizeof(hab_driver.settings));

	result = hab_parse(&hab_driver.settings);
	if (result) {
		HAB_LOG_ERR("hab config open failed, prepare default gvm 2 settings...\n");
		fill_default_gvm_settings(&hab_driver.settings, 2, MM_AUD_START, MM_ID_MAX);
	}

	/* now generate hab pchan list */
	result  = hab_generate_pchan_list(&hab_driver.settings);
	if (result) {
		HAB_LOG_ERR("failed to generate any pchan list! return %d\n", result);
	} else {
		int pchan_total = 0;
		for (i = 0; i < hab_driver.ndevices; i++) {
			device = &hab_driver.devp[i];
