...
hab_msg_alloc(struct physical_channel *pchan, size_t sizebytes)
{
	...
	if (!message) {
	    HAB_LOG_ERR("Out of memory! failed to allocate %d bytes\n",
		...
		struct uhab_context *ctx, size_t sizebytes)
{
	...
	if (sizeof(ack_recvd->ack) != sizebytes)
		HAB_LOG_ERR("fatal error! exp ack size %d is different than arrived %d\n",
		...
		struct hab_header *header)
{
	...
	if (payload_type != HAB_PAYLOAD_TYPE_INIT &&
		...
		if (payload_type >= HAB_PAYLOAD_TYPE_MAX ||
			...
			!vchan_id ||
			!session_id ) {
			HAB_LOG_ERR("Invalid message received! payload type %d, vchan id %x, sizebytes %zx, session %d\n",
						payload_type, vchan_id, sizebytes, session_id);
		}

		vchan = hab_vchan_get(pchan, header);
		if (!vchan) {
			HAB_LOG_INFO("vchan is not found! payload type %d, vchan id %x, sizebytes %zx, session %d\n",
						payload_type, vchan_id, sizebytes, session_id);

			if (sizebytes)
				HAB_LOG_ERR("payload type %d is dropped due to missing vchan, session id %d\n", payload_type, session_id);
			return;
		} else if (vchan->otherend_closed) {
			hab_vchan_put(vchan);
			HAB_LOG_INFO("vchan remote is closed! payload type %d, vchan id %x, sizebytes %zx, session %d\n",
						payload_type, vchan_id, sizebytes, session_id);
			if (sizebytes)
				HAB_LOG_ERR("payload type %d is dropped due to remote close, session id %d\n", payload_type, session_id);
			return;
		}
	} else {
		if ( sizebytes != sizeof(struct hab_open_send_data) ) {
			HAB_LOG_ERR("Invalid open request received! payload type %d, vchan id %x, sizebytes %zx, session %d\n",
						payload_type, vchan_id, sizebytes, session_id);
		}
	}
	...
	case HAB_PAYLOAD_TYPE_ACK:
		...
		if (ret) {
			HAB_LOG_ERR("failed to handle %d during open, payload size %d\n",
	...
	case HAB_PAYLOAD_TYPE_EXPORT_ACK:
		ret = hab_receive_create_export_ack(pchan, vchan->ctx, sizebytes);
		if (ret) {
	...
	case HAB_PAYLOAD_TYPE_CLOSE:
		...
		HAB_LOG_INFO("remote side request close vchan id %X, other end %X, session %d\n",
	...
	case HAB_PAYLOAD_TYPE_PROFILE:
		...
		message = hab_msg_alloc(pchan, sizebytes);
		if (!message) {
			HAB_LOG_ERR("failed to allocate msg!Arrived msg will be lost!\n");
			break;
		}
	...
	default:
		HAB_LOG_ERR("unknown msg is received! payload type %d, vchan id %x, sizebytes %zx, session %d\n",
						payload_type, vchan_id, sizebytes, session_id);	
		break;
	}
	if (vchan)
