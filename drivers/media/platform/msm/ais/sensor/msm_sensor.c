...
static u32 msm_sensor_evt_mask_to_sensor_event(u32 evt_mask)
{
	...
	switch (evt_mask) {
	case SENSOR_EVENT_MASK_INDEX_SIGNAL_LOST:
		evt_id = SENSOR_EVENT_SIGNAL_LOST;
		break;
	case SENSOR_EVENT_MASK_INDEX_SIGNAL_VALID:
		evt_id = SENSOR_EVENT_SIGNAL_VALID;
		break;
	...
	struct v4l2_event_subscription *sub, bool subscribe_flag)
{
	...
	for (evt_mask_index = SENSOR_EVENT_MASK_INDEX_SIGNAL_LOST;
