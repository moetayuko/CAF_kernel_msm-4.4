...
static void bridge_irq_delay_work(struct work_struct *work)
{
	...
	/* Get INTERRUPT_STS status */
	rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
		sensor_i2c_client, 0x24,
		&int_sts, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
		pr_err("%s: %s: read id failed\n",
			__func__, sensor_name);
		goto error;
	} else {
		pr_err("msm_sensor_driver: intsts=%d", int_sts);
	}

	/* Fill the sensor event */
	sensor_event.sensor_slave_addr =
		slave_info->sensor_slave_addr;
	sensor_event.sensor_device_id =
		s_ctrl->sensordata->cam_slave_info->camera_sensor_device_id;
	/* Queue the event */
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	pr_err("s_ctrl[%d] %pK", slave_info->camera_id, s_ctrl);
...
CSID_TG:
	...
	*/
	...
	s_ctrl->gpio_array[0].gpio = slave_info->gpio_num;
